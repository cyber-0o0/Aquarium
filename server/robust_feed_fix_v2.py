...
# Unified script with PATH fixes
unified_script = r'''
import os
import sys
import re
...
sftp = ssh.open_sftp()
with sftp.file('/tmp/unified_feed_fix_v2.py', 'w') as f:
    f.write(unified_script)
sftp.close()

print("🚀 Executing unified fix with PYTHONPATH...")
stdin, stdout, stderr = ssh.exec_command("cd /root/aquarium-ai/server && export PYTHONPATH=. && venv/bin/python /tmp/unified_feed_fix_v2.py")
print(stdout.read().decode())
print(stderr.read().decode())

print("🔄 Restarting aquarium (Final)...")
