...
async def handle_update(update: Dict[str, Any], db, source: str = "webhook") -> None:
    ...
    if not agent:
        # Fallback: if private chat, maybe create/find agent for this user
        ...
        if not user: return # User must be registered via Mini App first
        
        # Find any agent for this user to chat with in DM
        res = await db.execute(select(AgentModel).where(AgentModel.user_id == user.id).limit(1))
        agent = res.scalar_one_or_none()
        if not agent:
            # Create a default agent for PM
            import uuid
            agent = AgentModel(
