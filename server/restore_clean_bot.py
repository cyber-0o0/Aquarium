...
# Local clean content (reconstructed from view_file and our fixes)
# I will use the exact content from the previous view_file (Step 1005) 
# but I'll make sure handle_update has the private chat fix.

ssh = paramiko.SSHClient()
ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
ssh.connect(hostname, port=22, username=username, password=password)

# Define the CLEAN content of the file
clean_code = r'''"""
...
# Escape single quotes and write the file
with open('clean_telegram_bot.py', 'w', encoding='utf-8') as f:
    f.write(clean_code)

# Upload and move to target
print("📤 Uploading clean telegram_bot.py to server...")
sftp = ssh.open_sftp()
sftp.put('clean_telegram_bot.py', '/tmp/telegram_bot.py')
sftp.close()

print("🛠️ Moving clean file to destination and restarting...")
...
print("✅ Server restored and restarted with CLEAN code.")
ssh.close()
os.remove('clean_telegram_bot.py')
